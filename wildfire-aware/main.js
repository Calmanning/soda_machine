require([
	...
	console.log(ENV);
	//Query URLs
	const aggregatePerimeterURL =
		ENV === 'livingatlasdev.arcgis.com'
			? config.developmentEnv.urls.aggregatePerimeterURL
			: config.productionEnv.urls.aggregatePerimeterURL;
	...
	const setFireContentInfo = ({ fireData }) => {
		...
		const fireIcon = () => {
			if (fireData.personnelAssigned && fireType === 'PRESCRIPTION FIRE') {
